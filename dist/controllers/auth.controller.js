...
function getCookieOpts() {
    ...
    const sameSite = domain ? "none" : (process.env.NODE_ENV === 'production' ? "none" : "lax");
    const opts = {
        httpOnly: true,
        secure: secure,
        sameSite: sameSite,
...
const refreshController = async (req, res) => {
    ...
    try {
        ...
        const newRefreshRaw = crypto_1.default.randomBytes(64).toString('hex');
        const newHashed = crypto_1.default.createHash('sha256').update(newRefreshRaw).digest('hex');
